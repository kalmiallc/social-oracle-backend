...
export async function downgrade(queryFn: (query: string, values?: any[]) => Promise<any[]>): Promise<void> {
  await queryFn(`
    ALTER TABLE \`${DbTables.OUTCOME_SHARE_TRANSACTION}\` 
      DROP CONSTRAINT \`${FK_OUTCOME_SHARE_TRANSACTION_PREDICTION_SET_ID}\`,
      DROP CONSTRAINT \`${FK_OUTCOME_SHARE_TRANSACTION_OUTCOME_ID}\`,
      DROP CONSTRAINT \`${FK_OUTCOME_SHARE_TRANSACTION_USER_ID}\`
  `);
}
